...
// import startRestApiMetricCollection from "./apis/GrafanaFunctions.jsx";
...
const GrafanaLogs = ({ subTab }) => {
  const { cname, setCname, apiUrl } = useGlobalState();
  const {
    loadingGrafana,
    setLoadingGrafana,
    grafanaFormInputs,
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      //   grafanaFormInputs.input_start_date,
      //   grafanaFormInputs.input_end_date,
      //   metricName,
      //   saasEnv,
      //   apiRegex,
      //   cname,
  ...
  const [startDateDefault, startTimeDefault] =
    grafanaFormInputs.input_start_date.split("T");
  const [endDateDefault, endTimeDefault] =
    grafanaFormInputs.input_end_date.split("T");
  const defaultRange = [
    dayjs(startDateDefault + "      " + startTimeDefault),
    dayjs(endDateDefault + "      " + endTimeDefault),
  ];
  return (
    <div className="container">
      ...
      <form onSubmit={handleSubmit} className="input-form gap-4">
        <div className="flex flex-row allign-center flex-grow">
          <span className="px-3 py-2 bg-gray-50 border border-gray-300 border-r-0 rounded-l-md font-semibold">
